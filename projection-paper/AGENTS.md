...
2. **Inline citations are `[@citation_key]` keys, not typos or placeholders.**
   Resolve each key against **`2026m.bib`** (BibTeX). A `[-@key]` is the same key
...
3. **To get a formatted paper** (numbered or author-year references):
   `pandoc paper.md --citeproc --bibliography=2026m.bib --csl <style>.csl -o paper.pdf`
...
| `2026m.bib` | BibTeX for **every work cited** in `paper.md` (the cited subset of the corpus). | Resolve any `[@key]` here; 1:1 with the keys used in the paper. |
| `CITATIONS.md` | How the `[@key]` ↔ `2026m.bib` resolution works. | Read first if unsure how citations render. |
...
`2026m.bib` to get the real source — do not treat the key as the source itself.
