...
Future research directions include: automated test generation (can an LLM generate process contracts from signal requirements, or procedures from contracts, making orgschema a generative methodology?); regression testing for business changes (when a supplier changes, which customer experience tests might break?); multi-location test variance (should different locations share identical acceptance tests or adapt them to local cohorts?); and integration with ISO 9001:2026, which introduces requirements for digital quality management that align with orgschema's specification-first approach.[^24]
