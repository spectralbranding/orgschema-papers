...
The contribution is best stated as a conjunction of four properties. An acceptance contract is *ex-ante* if it is verified before and during deployment rather than after; it is at the *experience layer* if the criterion it encodes is a stakeholder-defined outcome rather than an internal process metric; it is *machine-checkable* if satisfaction is decided automatically rather than by human inspection at an audit; and it is *backward-traceable* if every operational parameter is formally linked to at least one such criterion. Table 1 places six established traditions against these four properties. The axis is not neutral and is not offered as one: the four properties are exactly the conjunction this paper contributes, so OST satisfies them by construction and Table 1 is a positioning table rather than an evaluation. Its work is to make the axis explicit and locate each tradition on it, so that a reader can see what the artifact commits to and dispute the axis if they wish. The claim is not that any tradition is deficient in isolation—each is mature and effective at what it does—but that none supplies the four jointly, and that the reason in each case is structural rather than an oversight. That last point is what the section can defend, because it does not depend on who drew the axis.

**Table 1: Positioning of Six Adjacent Traditions Against the Four-Property Axis.**
...
*Notes*: ✓ = provided by design; ~ = partial or domain-limited; — = absent. Ratings are structural judgments about each tradition's design intent, not measurements of any deployment; they are argued individually in the text below. The axis is this paper's own contribution restated as criteria, so the OST row holds by construction; the table positions the traditions against the artifact's design commitments and is not an evaluation of them, and no ranking is intended.
...
A four-vertex example makes this concrete. Let the graph contain one experience contract $e$ at L0 ("coffee served within three minutes"), one process contract $p$ at L2 ("extraction completes in 25–30 s") with an edge $p \to e$, one procedure $d$ at L3 ("tamp with 15 kg force") with an edge $d \to p$, and one further procedure $w$ at L3 ("engrave the barista's initials on the portafilter") with no outgoing edge. Reverse reachability from the L0 sink $e$ marks $\{e, p, d\}$ as justified: each reaches $e$ by following arrows. The remaining set $V \setminus R = \{w\}$ is the specification waste—engraving initials traces to no stakeholder outcome, so the pipeline flags it, not because it is forbidden but because nothing in the specification explains why it is there. Adding an edge from $w$ to some genuine outcome removes it from the waste set, but that is an edit to the specification rather than a test of the identification. The identification is tested by whether a vertex the graph leaves unreachable is in fact required for an L0 outcome as the specification stands. The statements below generalize exactly this picture.
...
Following the FEDS framework [@venable-2016-feds], the artifact is assessed through three strands: an analytical argument, independent naturalistic corroboration, and a public demonstration. Only the latter two confront the artifact with something outside it and are evaluation in that sense; the analytical strand is positioning and derivation, and is labelled as such below. We make no causal or KPI-superiority claim: we do not claim that OST deployments outperform ISO 9001, TOGAF, or any other practice on any measured outcome. The evaluation establishes that the artifact has the properties it is designed to have and that an independent organization converged on the same design logic—not that it improves performance.

### 8.1 Analytical positioning and derivation

The systematic comparison of Section 2 (Table 1) is positioning, not evaluation. Against the four-property axis, OST is the only artifact for which ex-ante, experience-layer, machine-checkable, and backward-traceable hold jointly—but the axis is the artifact's own contribution stated as criteria, so that result is a consequence of how the axis was drawn and carries no independent evidential weight. What the comparison establishes is the shape of the claim: which properties are being asserted, which traditions supply which of them, and why each shortfall is structural rather than incidental.

The derivation that does carry content is Section 5, which shows why the joint property is irreducible under asymmetric verifiability; that result concerns the graph model, not the traditions in Table 1. Restated as a graded claim: the artifact raises machine-checkable experience-linkage from the small fraction of parameters that adjacent traditions link to an experience criterion toward near-complete coverage, and reduces validation latency from the periodic cadence of audit to continuous per-change checking. Both are analytical consequences of the design principles and the formal model—independent of any deployment, and independent of Table 1. The evaluative weight of this section therefore rests on the naturalistic corroboration in 8.2, with the demonstration in 8.3 establishing feasibility only.
