...
2. **Inline citations are `[@citation_key]` keys, not typos or placeholders.**
   Resolve each key against **`multi-llm-coding-instrument.bib`** (BibTeX). A `[-@key]` is the same key
...
3. **To get a formatted paper** (numbered or author-year references):
   `pandoc paper.md --citeproc --bibliography=multi-llm-coding-instrument.bib --csl <style>.csl -o paper.pdf`
...
| `multi-llm-coding-instrument.bib` | BibTeX for **every work cited** in `paper.md` (the cited subset of the corpus). | Resolve any `[@key]` here; 1:1 with the keys used in the paper. |
| `CITATIONS.md` | How the `[@key]` ↔ `multi-llm-coding-instrument.bib` resolution works. | Read first if unsure how citations render. |
...
`multi-llm-coding-instrument.bib` to get the real source — do not treat the key as the source itself.
