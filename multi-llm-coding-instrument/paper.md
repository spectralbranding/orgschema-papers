...
The instrument is specified for discretely-codable variables over evidence that can be assembled into a self-contained, source-cited dossier. It is not specified for open-ended free-text extraction, for continuous ratings lacking a stated discretization rule, or for evidence that resists dossier assembly.

That first exclusion is the condition the reliability rests on rather than a modest disclaimer, and evidence outside this paper now carries the weight. Where machine operators are asked to recover a structure from prose with no pre-committed schema — so that which categories exist is itself at issue — agreement between operators from different families has failed under a pre-registered design built to rescue it: edge-level agreement fell short of its declared gate in every document-by-condition cell, including those in which the unit inventory was fixed and supplied identically to both operators, and separated below the operators' own within-operator noise floor [@zharnikov-2026bl-unitization-before-extraction]. That study is not a validation of this instrument and shares neither its corpus, its coefficient, nor its task. The pairing matters for a different reason: an almost-perfect coefficient on one corpus could be a property of that corpus or of general model competence, whereas the contrast between coding into a schema committed before any datum exists and recovering a schema that is itself in question identifies which feature of the design the reliability is attributable to. Practitioners should read the boundary accordingly — the discipline package transfers to bounded coding tasks, and it should not be assumed to transfer to open-ended extraction merely because the same models perform both. Its coders are exact model versions, and its results are conditional on those versions; as models are retired and replaced, coded values are expected to drift. Pinned provenance and the multi-model majority mitigate that drift — a re-run records exactly which versions produced which codes, and a single model's change cannot silently move a majority cell — but they do not eliminate it, and periodic re-validation is part of responsible use.
