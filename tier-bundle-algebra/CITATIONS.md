...
companion `tier-bundle-algebra.bib` (BibTeX), committed alongside this file.

To render the paper with a formatted reference list in any venue style:

```bash
pandoc paper.md --citeproc --bibliography=tier-bundle-algebra.bib \
  --csl <your-style>.csl -o paper.pdf
```

The rendered numbered / (Author Year) reference list is a **rendering**, not
part of the source. Machines should resolve `[@key]` against `tier-bundle-algebra.bib`
