...
companion `succession-rotation.bib` (BibTeX), committed alongside this file.

To render the paper with a formatted reference list in any venue style:

```bash
pandoc paper.md --citeproc --bibliography=succession-rotation.bib \
  --csl <your-style>.csl -o paper.pdf
```

The rendered numbered / (Author Year) reference list is a **rendering**, not
part of the source. Machines should resolve `[@key]` against `succession-rotation.bib`
